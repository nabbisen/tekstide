...
| 054 | User Configuration Completion | M12 | 023, 045 | no | **Proposed 2026-09-24.** `0.25.0`, and **M12 closes with it**. Keybindings, theme colours, font family, font size and terminal scrollback become settable; `REQ-CONFIG-006`/`007`, `NFR-UX-004`, `REQ-TERM-004`. Measured first: the document has two sections and nothing for the UI, `Theme` is already one seam, `KeybindingRule` already carries a `Configurable` status nothing reads, and `SCROLLBACK_LINES` is a constant. **D2 pins that no project-local configuration is ever read**; D3 refuses a conflicting or reserved rebind rather than taking last-wins; D5 validates contrast; D6 takes a font *name*, never a path |
