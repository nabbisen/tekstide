...
| 044 | Surface-Local Keyboard Affordances | M12 | 039, 040, 016 | no | **Proposed 2026-08-26.** Two keyboard systems, one accountable: 14 global bindings with a registry, a coverage test and generated help; ~29 surface-local keys across eight handlers with none of that. Three surfaces have needed a mouse-only control fixed, each caught by a reviewer rather than a test. D1 weighs a registry against the fact that `Enter` means six different things on purpose |
...
| 043 | Terminal Process Containment | M12 | 008, 009, 013 | **yes** | **Accepted 2026-08-26**, D1–D4 decided on acceptance. Kill, session-scoped rather than by cgroup — a cgroup would also kill what a user detached with `nohup`/`setsid`, and the session is the boundary that makes killing safe. The sequence changes: SIGHUP the session leader first so the shell hups its own jobs; today's SIGKILL-first destroys that. The audit field may claim only what a real re-enumeration saw. The leak guard lives in `RunningTerminal::drop`, not per test — the audit-store slice proved per-site wiring misses sites |
