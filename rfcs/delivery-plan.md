...
| 036 | Dormant Capability Closure | M12 | — | no | **Accepted 2026-08-18; D0–D4 decided 2026-08-27** when its pack was written (it predates the decide-on-acceptance convention). **D0: re-verify before triaging** — the list is ten days old and `request_terminate` now has three production callers. **D1: deletion is on the table, batched into `0.16.0`**, which RFC-044 already owes — deletion is recoverable from git, a wrong "keep" is invisible forever. **D2: "core-only" needs a named consumer that exists as a file**, which required reserving RFC-045 for the four configuration-conditioned rows. **D3: `recover`/`purge_all_records` leave the triage** — a recovery path that never runs is a defect, not dormancy. **D4: each row names the search that would have found it.** Pack: `handoffs/036-dormant-capability-closure/README.md` |
