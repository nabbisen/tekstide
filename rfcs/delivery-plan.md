...
| 049 | Transcript Retention Enforcement | M12 | 011, 033, 045 | **yes** | **Accepted 2026-09-12; D1–D9 decided on acceptance.** Of RFC-011's four retention budgets only the per-transcript byte cap is enforced: the per-project and app-wide budgets are computed into `budget_pressure` and read by nothing, `max_age_days` is read only by an `is_bounded()` check and RFC-045's config write, and `TranscriptRetentionState::Expired` has no producer. RFC-011's Default Retention already specifies the cleanup and the `RequiredLocalBounded` preflight refusal; its Acceptance Criteria pair "bounded" with "accounting", which is the ambiguity the implementation resolved the weaker way. Security-relevant: this is the first slice here that **deletes user data**, and the first case where a retention budget changes what a launch does — **D4′** (2026-09-13): capture disabled for that run and disclosed, since no product path requests `RequiredLocalBounded`. **PR-049-C partly paused (request 387):** a session knows only transcripts launched in the current process, which also means RFC-033's purge has covered nothing from earlier runs since `0.12.0`; loading transcripts from disk is RFC-050, accepted 2026-09-13 |
| 050 | Transcripts From Earlier Runs | M12 | 011, 033, 049 | **yes** | **Accepted 2026-09-13; D1–D9 decided on acceptance.** Ahead of RFC-049 PR-049-C, which it unblocks. A `ProjectSession` knows only transcripts launched in the current process: RFC-033's purge and the retained figures have covered nothing from earlier runs since `0.12.0`. Load records from disk at project open (one loading path, not a purge that walks the directory); the writer holds a lock and a load-time probe reads it, so a second instance's live file is never treated as a leftover; mtime read once at load gives the age. **D6′**: a corrupt `recent-projects.json` silently resets every project id, so unclaimable directories can be every transcript a user has — counted and shown, never deleted in-app. Security-relevant: it widens what purge and retention can reach. Slices A → B → C |
