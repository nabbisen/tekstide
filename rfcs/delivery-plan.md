...
| 048 | AgentRun Termination Records | M12 | 046, 043, 013 | **yes** | **Accepted 2026-09-16; D1–D6 decided on acceptance.** Reserved by RFC-046 D3, which recorded the launch and declined to record the ending until the terminating paths were inventoried — they are, in the RFC. The trail cannot say whether an AI CLI run ended. **No schema change**: the `Terminated` phase is already permitted and already ordered by the store. **D2**: a detached run gets no record, and the limit is disclosed. **D5**: the coordinator applies the outcome and records it in one path, because two prior slices shipped with an unguarded call site |
