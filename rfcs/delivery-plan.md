...
| 030 | Git Integration | M12 | 012 | **yes** | **Accepted 2026-09-22; D1′ decided 2026-09-22 (review 405).** Slice A measured both candidate mechanisms against an adversarial repository and **both ran a repository-named clean filter**; `git` also ran `core.fsmonitor`. A repository that names nothing executes nothing, so the answer is a hardened subprocess **behind a gate that refuses any repository that asks anything of us** — allowlist, unknown key refuses, `include`/`includeIf` refuses, configuration read with `git config --list` because `--local` hides an include-supplied driver. Refusal is "not available", the words RFC-025 D5 already renders. Read-only in every trust state; the gate, not the trust grant, is what makes it safe. Ships when its evidence is done, as `0.22.0` |
