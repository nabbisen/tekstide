...
| 056 | AgentRun Report And Classification | agent remainder | — | partly | **Accepted 2026-09-25; D1–D8 as written, plus D9–D12** — the record is written on every change not only at run end, it must be reachable and captured live, no thirteenth audit family (open question, below), and a killed run's ending is genuinely unknown. [Handoff pack](./handoffs/056-agentrun-report/README.md). Four slices: **A** the `0.26.0` pin fix, alone and first; **B** the record; **C** purge takes it; **D** the classification, the notes and the report. `0.27.0`. **D1: a run leaves a `run.json` in the `agent-run-<id>/` directory it already owns** — measured, the id is the only fact about a run that survives the process, and the transcript loader already ignores every file but the transcript. **D2: purge takes the record with the transcript**, because purge removes only the transcript file today and a record naming the prompt, the changed paths and the user's notes would outlive it. D5: the report is generated, never stored twice. [The RFC](./accepted/056-agentrun-report-and-classification.md). Carries the `0.26.0` post-publish fix (pin the core by version) as its first slice. **Reserved 2026-09-24.** `0.27.0`. `REQ-AGENT-011` and `015`, both recorded as implemented in this plan until the audit checked: no report, no handoff note, and `AgentRun` carries no classification field at all |
