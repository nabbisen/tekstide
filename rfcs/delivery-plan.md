...
| Configuration system | `REQ-CONFIG-001`..`007` | **Complete as of RFC-054 (2026-09-24).** The mechanism was RFC-023's and RFC-045 reached it; **RFC-054 made keybindings, theme colours, font family and size, and terminal scrollback settable**, closing `REQ-CONFIG-006`, `007`, `NFR-UX-004` and `REQ-TERM-004`. Each bad value falls back on its own with the board naming it; `Ctrl+Alt+C` applies all of it live. `transcript_retention_days` is enforced (RFC-049) |
...
| **`.gitignore` in the explorer, and an `ignored` badge** — the scanner collapses a fixed directory list; `git status` is invoked without `--ignored` | `REQ-FILE-005`, part of `002` | **Implemented by RFC-055 (closed 2026-09-25; `0.26.0`)** — `REQ-FILE-005` (except `core.excludesFile`), `REQ-FILE-006` and the `ignored` category of `002` |
| **AgentRun report/handoff note, and run classification** | `REQ-AGENT-011`, `015` | **Implemented by RFC-056 (closed 2026-09-25; `0.27.0`)**, with evidence a user can reach both: `handoffs/056-agentrun-report/evidence/pr-056-d/` |
...
project-derived paths outside the state directory. RFC-031 completed the twelve audit families and
every one has a producer; a thirteenth brings its own retention and purge obligations, and RFC-033's
rule that a record names no path would leave it saying "a report was exported" without saying where —
which is most of what would make it worth having. Revisit with RFC-059 (*Seeing The Audit, And
Redacting It*, `0.32.0`), where the question of what a record is *for* is already on the table.
...
| 017 | Terminal Renderer and Immersion Mode | M9 | 014, 015 | no | **Implemented and closed.** In `rfcs/done/`. *(Row corrected 2026-09-16: it still read "Accepted 2026-08-01 — next up" long after the RFC closed — the same state-asserting drift this project keeps finding, in the file that answers "what is startable work".)* |
...
| 049 | Transcript Retention Enforcement | M12 | 011, 033, 045 | **yes** | **Implemented and closed 2026-09-16.** Moved to `done/`. Three slices; **C** split into the independent list, the triggers and disclosures, and a follow-up. The first slice here that deletes a user's data without being asked: two triggers a user causes, no timer. **D8′** (liveness from `AgentRun.status`), **D4′** (an exhausted budget disables capture for that run and says so), **D6** (policy removals audited distinctly from a user's purge). Review-found: the adapter launch site nothing tested, a cleanup that failed at everything recording nothing, and two tests that did not hold the property they were filed under. Blocked mid-slice by RFC-050, which had to make purge reach earlier runs first |
| 048 | AgentRun Termination Records | M12 | 046, 043, 013 | **yes** | **Implemented and closed 2026-09-16.** Moved to `done/`. Two slices. The trail states how a run ended and states nothing for a detached one. No schema change: the phase and its ordering already existed. **Found during implementation**: the producer existed with only test callers, and production dropped the launch's operation id — both architect errors of the same shape (asserting from shape, not callers) |
| 051 | Recovering the Recent-Project List | M12 | 032, 047, 050 | **yes** | **Implemented and closed 2026-09-16.** Moved to `done/`. Three commits: the store, the product, and the deletion of the method the first commit made dormant. Quarantine before overwrite; recover from one previous-good copy, written only from a list that loaded; whole-file or nothing; recovered and reset are different sentences. Trust is still re-verified against the audit store, so recovery restores identities and never grants. Ships with RFC-048 in `0.20.0` |
| 050 | Transcripts From Earlier Runs | M12 | 011, 033, 049 | **yes** | **Implemented and closed 2026-09-16.** Moved to `done/`. Three slices, in order: the writer's lock and a transcript's origin; loading from disk with purge and the figures following it; and the notices. Fixes the purge defect shipped from `0.12.0` through `0.18.0`, disclosed at `c2f5092` and corrected in the changelog when the fix landed. **D3′**: purge skips a file only when this session does not own its writer. **D6′**: unclaimable directories come from a corrupt `recent-projects.json` resetting every project id. Review-found follow-ups: the adapter launch site's guard, three notice wordings |
...
| 052 | A File Explorer A User Can Read | M12 remainder | 030 | no | **Implemented and closed 2026-09-24.** Moved to `done/`. `0.24.0`. Four slices: **A** the hostile fixture and D3′ by measurement (the candidate widget draws raw bidi names, has no badge slot and lays out 100 000 rows in 2.2 s; `ItemTree` ruled out because it hardcodes the font size RFC-054 makes configurable); **B** the tree — expansion in place, the scan off the render thread, virtualisation rather than a cap, nothing hidden silently; **C** folders first, text-symbol icons, `[open]`, monospace; plus an owner-requested **Project Board** rework carried alongside. Three defects found by live capture and none by unit tests, including a scroll request set and never consumed |
| 053 | What The Window Says Is True | M12 remainder | — | no | **Implemented and closed 2026-09-24; released as `0.23.0` (`dfc5208`).** Closed three releases late — it sat in `accepted/` through `0.24.0` and `0.25.0` and was caught while measuring for RFC-055; the lapse and the check that would have caught it are in the RFC's Closed section. **Accepted 2026-09-24 with the schedule; D1–D9 decided on acceptance.** `0.23.0`, and the first thing built under the authorised schedule. Six measured defects, each the product saying something untrue: a shipped `RFC-017` placeholder, modals clipping their own `Close` away at 760×560, a status bar wrapping at 520px against the one-line invariant `content_area_height` uses to size every PTY, Change Review contradicting the Git state in one frame, `unknown` where zero is known, and two facts both called dirty/changed. D3 measures the rendered bar height; D9 adds the internal-identifier scan, with doc comments staying legal |
| 054 | User Configuration Completion | M12 | 023, 045 | no | **Implemented and closed 2026-09-24. M12 closes with it.** `0.25.0`. Three slices: **A** keybindings and the `Configurable`/`None` repair (`RuleBinding` a three-variant sum, `Dead` carrying a reachability claim, a collision refusing every participant to a fixed point); **B** theme colours at 4.5:1 with the measured ratio, font family by name looked up in the renderer's own database, bounded sizes, the live font via one recorded process-global; **C** the scrollback cap **measured** — 12,000 lines, fewer on a wider pane, the allocator probe catching the author's model wrong three times |
| 055 | Ignore Rules In The Explorer | M12 remainder | 052 | no | **Implemented and closed 2026-09-25.** `0.26.0`. [Handoff pack](./handoffs/055-explorer-ignore-rules/README.md). Three slices and a follow-up to each. The explorer collapsed a three-name array and read no `.gitignore`; it now asks git which entries of each folder are ignored (`check-ignore`, bounded by the rows drawn, every path prefixed `./` at one function — a file named `:(glob)evil.log` aborts a batch) and marks them `[ignored]`. **The acceptance's budget was 50× wrong**: the full gate also walks every `.gitattributes` (~87 ms here, 96 % under `target/`), so the query asks only the gate's *configuration* half; and **`check-ignore` runs `core.fsmonitor`**, which is why a repository the gate does not accept is never asked. Unknown is a value twice (`IgnoreAnswer`, `ExplorerIgnoreState`) and the scan carries which rule decided. `explorer.show_ignored` (default off) hides ignored *files*, counted; an ignored *folder* keeps its row; dotfiles are never hidden. The entry's name now comes before its words (review 432 Q1). **`REQ-FILE-005` is implemented with one sentence attached: `core.excludesFile` is not honoured (RFC-062).** `REQ-FILE-006` and the `ignored` category of `REQ-FILE-002` are implemented plainly |
| 056 | AgentRun Report And Classification | agent remainder | — | partly | **Implemented and closed 2026-09-25.** `0.27.0`. [Handoff pack](./handoffs/056-agentrun-report/README.md). Four slices and a follow-up: **A** the `0.26.0` pin fix, alone and first — the finding it was written from was half wrong and the implementer corrected it against the registry (`--locked` is a working remedy; the defect is `version = "0"` alone); **B** the record — `agent-run-<id>/run.json`, versioned, atomic, written on every change; **a restored run is held apart from every lifecycle reader** (the launch limit, change attribution and the running/failed counts would each have had to remember it) and a run killed with the app says it does not know its ending; **C** purge takes the record by exact name and removes the directory only if it is then empty — the third-file test (`run.json.corruption-notes`, which a prefix match eats) was written before the code that deletes; **D** the classification, the notes and a report that keeps the user's words, Tekstide's facts and the run's text apart *in the file*. **Retention leaves the record** (review 438) and the purge dialog names runs, transcripts and records; the transcript quote stays in the export by default (review 439). **Captured live with no environment variable** — classify, note, export, close with the run still going, reopen, purge. `REQ-AGENT-011` and `REQ-AGENT-015` are implemented. **D11 stays open**: whether an export deserves an audit record, below |
| 057 | Editor Essentials | M10 remainder | — | no | **Accepted 2026-09-26; D1–D7 as written, plus D8–D11** — a drawn line stays one assertable string, the viewport follows the cursor (no scrolling invented here), D7 is `shell.rs:7520–7522`, and **D11 measures `NFR-PERF-003` before anything changes**. [Handoff pack](./handoffs/057-editor-essentials/README.md). Five slices: **A** the baseline, **B** rows bounded by the viewport, **C** gutter and caret, **D** undo, **E** the two-row header. `0.28.0`. Measured: the body is `document.text().to_string()` into one `text` widget, so the whole file is laid out every frame; `TextViewport { first_visible_line }` has existed since RFC-006 with **no reader**; every keystroke returns a whole-document `String`. **`NFR-PERF-003` (100 000-line typing latency, p95 ≤ 16 ms) has never been measured** and D6 measures it whatever the answer. D2: the caret is a widget, never a character — a caret glyph would be indistinguishable from the same character in the file. D7 carries the owner's two-row header as its own slice, because it is the only scheduled work touching the frame. [The RFC](./accepted/057-editor-essentials.md). **Reserved 2026-09-24.** `0.28.0`. A line-number gutter (`REQ-EDIT-002`'s unmet half), a visible caret, and **undo** — which no requirement names and `NFR-REL-005` assumes. Syntax highlighting (`REQ-EDIT-003`) stays an owner decision, not a silent deferral |
