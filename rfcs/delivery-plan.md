...
| 042 | Change Content Legibility | M12 | 041, 024, 018 | no | **Proposed 2026-08-26**, first of three for `0.15.0`. Found by the `0.14.0` release gate, not the suite: `quote_untrusted` escapes every line break in a previewed file, so a real source file renders as one line. The escaping is correct — the undecided question is whether an untrusted file may occupy N visual lines in trusted chrome, and what bounds N. Three decisions required before any code. **RFC-034 is blocked on it** |
