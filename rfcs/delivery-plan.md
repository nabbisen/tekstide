...
| 030 | Git Integration | M12 | 012 | **yes** | **Accepted 2026-09-22; D2–D8 decided on acceptance, D1 left open by design.** Scoped alongside RFC-025 and shipped when its evidence is done, not on `0.21.0`'s date. Slice A is evidence and a decision only: an adversarial repository fixture that names a program to run during a status read, both mechanisms measured against it, and the mechanism chosen by the rule — the library only if it executes nothing the repository names, otherwise the hardened subprocess with RFC-012's gate met item by item, otherwise the RFC stops and Git state keeps saying "not available". Read-only in every trust state |
| 025 | Notifications | M12 | 023 | partly | **Accepted 2026-09-22; D1–D7 decided on acceptance.** Headline of `0.21.0`. One `Notification` model replacing four ad-hoc board notice mechanisms, each of which decided its own lifetime; the migration's acceptance is that their existing absent-when-false tests pass unmodified. Two lifetimes only — a third would have no producer. Then REQ-NOTIFY-002's status-bar fields and 003's actionable labels, from counts `ProjectRuntimeSummary` already carries |
