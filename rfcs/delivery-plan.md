...
| 030 | Git Integration | M12 | 012 | **yes** | **Proposed 2026-09-17.** Scoped alongside RFC-025 and shipped when ready, not on `0.21.0`'s date: a repository names programs Git runs during a status read, so RFC-012's safety gate is met — by a read-only library or a hardened subprocess, measured first — before any Git state is shown. Renumbered from 024 on 2026-08-12 |
