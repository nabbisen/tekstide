...
| 055 | Ignore Rules In The Explorer | M12 remainder | 052 | no | **Accepted 2026-09-24; D1–D8 as written, plus D9 — the gate is not cached across expansions — and a corrected per-expansion budget (gate + query, a few ms, not the query's 1 ms).** [Handoff pack](./handoffs/055-explorer-ignore-rules/README.md). Three slices: **A** the query and the `:(glob)evil.log` file that tries to silence a directory, **B** the scan carrying it with the floor list demoted, **C** `explorer.show_ignored`, the badge, and the invariant RFC-053's late closure asked for. `0.26.0`. Git is the oracle and Tekstide never parses `.gitignore` (D1); the query is `check-ignore`, bounded by the rows drawn, because `--ignored=matching` returned **20 000 records** for a one-line `*.log` rule and `traditional` paid 109 ms descending (D2); every path is prefixed `./` because a file named `:(glob)evil.log` aborts the whole batch with exit 128 (D3). **D6 corrects RFC-052 D7**: the fixed `[.git, node_modules, target]` list is demoted to a floor with a stated source, not replaced. `REQ-FILE-005`, `006`, and the `ignored` badge of `002` |
