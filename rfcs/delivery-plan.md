...
| **User-configurable keybindings, theme, fonts and scrollback** — M12's own scope names them; RFC-023 shipped the mechanism and RFC-045 reached it, but no key exists for any of these. `NFR-UX-004` rides on the same gap | `REQ-CONFIG-006`, `007`; `NFR-UX-004` | **Built by RFC-054 (2026-09-24), awaiting review and `0.25.0`** — `REQ-CONFIG-006`, `007`, `NFR-UX-004` and `REQ-TERM-004` reachable and shown live |
...
| 054 | User Configuration Completion | M12 | 023, 045 | no | **Accepted 2026-09-24; D3 corrected on acceptance.** `0.25.0`, and **M12 closes with it**. Keybindings, theme colours, font family and size, terminal scrollback. **D3′ repairs the category error `future-work.md` recorded**: `Configurable`/`None` means *dead*, not bindable — two surfaces shipped unreachable because the two states look alike. Rebindable is *has a default binding and is not `Reserved`*, a conflict refuses, and every action ends with a chord or an explicitly dead status. Contrast at 4.5:1 with the measured ratio in the diagnostic, a font is a name never a path, the scrollback cap measured against 64 MB a pane |
