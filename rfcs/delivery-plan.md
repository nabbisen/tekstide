...
| 042 | Change Content Legibility | M12 | 041, 024, 018 | no | **Accepted 2026-08-26**, first of three for `0.15.0`. Found by the `0.14.0` release gate, not the suite: `quote_untrusted` escapes every line break in a previewed file, so a real source file renders as one line. D1/D2/D3 decided on acceptance, each verified against the code first — pin the framing outside the scroll region; make a content line impersonating a Tekstide line unrepresentable rather than merely unlikely; refuse above a measured line bound instead of truncating. **RFC-034 is blocked on it** |
| 034 | Change Review Actions and Review State | M12 | 020, 012, **042** | no | **Accepted 2026-08-18; unblocked 2026-08-25; amended 2026-08-26.** Second of three for `0.15.0`. The amendment records that neither value this RFC was accepted for exists today — no audit family covers a user's opinion of generated code, and `ChangeSet` has no `Serialize`, so a decision dies with the process. D0/D1/D2/D3 all decided on scheduling: session-scoped and said so; `Accepted`/`Rejected` only; no audit record; a stale tree is disclosed, not blocking. **Added to this queue retroactively 2026-08-25**, having been accepted for a week without appearing here |
...
| 036 | Dormant Capability Closure | M12 | — | no | **Accepted 2026-08-18**, unscheduled. Wire / delete / document per orphan. Its list has grown since: `set_resource_limits`, `to_ai_cli_profile`, `ConfigStore`, the `sensitive_config_changed` producer, `close_project` (now discharged by RFC-039), `CycleVisibleTerminalSession`, `OpenSafeCloseDialog`. **Added to this queue retroactively 2026-08-25** |
