...
| 047 | Audit Store Corruption Recovery | M12 | 013, 004, 036 | **yes** | **Authored and proposed 2026-08-28**, prioritised first of the three reserved by RFC-036 because it is the container: until a broken store fails loudly, no audit claim this product makes can be relied on, including RFC-046's. Grounded in measurement: `recover`/`resume` have zero production callers, `AuditHealth` is built fourteen times and read never, and `open_audit_store` collapses every failure reason to one silence. D3 (make it detectable) and D1 (resume the safe case) are close to free; D2 (discard a user's audit history?) and D4 (is a working audit store a precondition for the actions it audits?) are the real design |
