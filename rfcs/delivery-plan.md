...
| 046 | Managed AgentRun Audit Trail | M12 | 013, 036 | **yes** | **Proposed 2026-09-06; authorized by the owner as the next RFC to scope.** Sequence **after RFC-047 PR-047-D** — both touch `AuditHealth` on the same path. **Reserved 2026-08-28 by RFC-036's triage.** `launch_managed_agent_run` and siblings write real `ManagedProcessLifecycle` records, are proven against the real store, and have zero production callers — so launching an AI CLI agent, the action trust and command approval exist to control, leaves no durable audit record. The crate README claimed otherwise until 2026-08-27. What to record and when is design, hence an RFC |
| 047 | Audit Store Corruption Recovery | M12 | 013, 004, 036 | **yes** | **Accepted 2026-08-28**, D1–D4 decided on acceptance. Four built-and-unconnected pieces, including a fourth found while deciding: the `AuditStoreRecovery` audit family and its `recovery_record()` are already written and uncalled. D2 changed on a checked fact — `recover()` is `fs::rename`, so it quarantines rather than deletes, which makes automatic recovery defensible provided the quarantined path is surfaced. D4 declines to refuse: Restricted Mode refuses actions whose danger it cannot bound, and a broken store makes an agent run unrecorded, not more dangerous — so the confirmations say so before the click instead |
