...
| 044 | Surface-Local Keyboard Affordances | M12 | 039, 040, 016 | no | **Accepted 2026-08-27**, D1–D4 decided on acceptance. Scope widened by `0.15.0`'s release gate, which found closing a project is mouse-only — `control_coverage`'s domain is `NavigationAction`, which closing is not, and `ControlCoverage` has no `MouseOnly` arm, so the gap was inexpressible. Registry keyed by (surface, key) over surface actions; Help grouped by surface rather than key hints on labels; the mirror of `control_coverage`, exhaustive; inventory first, then access, then advertising |
