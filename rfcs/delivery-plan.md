...
| 051 | Recovering the Recent-Project List | M12 | 032, 047, 050 | **yes** | **Accepted 2026-09-16; D1–D6 decided on acceptance.** Ships with RFC-048 in `0.20.0`. A `recent-projects.json` that cannot be read is overwritten with an empty list, so a user loses their project ids — and a grant is matched by id, so trust cannot be re-verified, and RFC-050 D6′'s orphaned transcripts are the same event seen from the transcript side. No backup exists today. **D1** quarantine before overwrite, **D2′** back up only a list that loaded, **D3** whole-file recovery, **D5** recovery and reset are different sentences, **D6′** one typed outcome from the store |
