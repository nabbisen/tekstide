...
| RFC | Title | Status |
| --- | --- | --- |
| 020 | [Diff Review and AgentRun Report Surfaces](./accepted/020-diff-review-and-agentrun-report.md) | **Both surfaces implemented and reachable.** AgentRun report surface implemented in full 2026-08-18, `Ctrl+Alt+R`. Change review surface implemented 2026-08-25, `Ctrl+Alt+D` — metadata only (file paths, count, detection status, review state), reviewed and accepted with one required closeout item, also delivered 2026-08-25; diff *content* rendering remains future work, blocked on retaining `DetectedChanges` past `add_detected_generated_change_set`. [Handoff pack](./handoffs/020-diff-review-and-agentrun-report/README.md); both surfaces' own slice handoffs and evidence in the same pack |
