...
| RFC | Title | Status |
| --- | --- | --- |
| 042 | [Change Content Legibility](./accepted/042-change-content-legibility.md) | **Accepted 2026-08-26. First of three for `0.15.0`; RFC-034 is blocked on it.** RFC-041 made a changed file's content inspectable and it is not **readable**: `quote_untrusted` escapes every line break, so a real source file renders as one line. Three decisions settled on acceptance, each checked against the code first. **D1: pin the frame** — today `scrollable(column(lines))` puts the disclosure and the content in one scroll region, so a long file scrolls the "not a diff" label off screen; a claim that qualifies content stays visible for as long as that content is. **D2: make impersonation unrepresentable** — chrome and untrusted content are one flat `Vec<String>` today, told apart by index, so a file line reading `Review state: Accepted` is indistinguishable from the real one; they become different types in their own containers. **D3: refuse above a line bound, never truncate**, matching RFC-024's byte precedent, with the bound measured rather than chosen and no third omission count. [Handoff pack](./handoffs/042-change-content-legibility/README.md) |
