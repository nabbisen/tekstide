...
| 023 | [Configuration System](./proposed/023-configuration-system.md) | **Accepted 2026-08-18.** Owns `set_resource_limits` (the reachability audit's priority 3), the per-role theme values `Theme` is a seam for, and the decision recorded in its own pack that the WCAG contrast gate does not survive configurability |
| 031 | [Audit Producer Completion](./proposed/031-audit-producer-completion.md) | **Accepted 2026-08-18**, authored against the number reserved 2026-08-16. Scoped to the two families with live triggers (`restricted_mode_blocked`, `project_added`); `safe_close_decision` is blocked on a dialog that does not exist. **Security core corrected 2026-08-18 before its pack was written**: `DurableAuditRecordV1` has no free-text field and `AuditReference` rejects `/`, so a path is unrepresentable — the residual risk is a single attacker-influenceable path segment reaching `subject_ref`, which both producers leave `None`. **PR-031-A and PR-031-B both implemented 2026-08-19, awaiting review** — restricted-mode refusals and project opens now write real records, each reachable from a real user action, not a dispatched command; restore-on-boot writes none (settled by reading `restore_recent_projects`, which never reaches `add_project_session`, correcting the pack's own task breakdown). [Handoff pack](./handoffs/031-audit-producer-completion/README.md) |
| 033 | [Transcript Lifecycle Controls](./proposed/033-transcript-lifecycle-controls.md) | **Accepted 2026-08-18.** Per-run opt-out, purge, and retained-data visibility — all designed by RFC-011, none reachable. Authored because `0.11.1` had to publish "there is no in-app way to turn capture off or to purge it" as a standing limitation |
| 034 | [Change Review Actions and Review State](./proposed/034-change-review-actions-and-review-state.md) | **Accepted 2026-08-18.** Gives `transition_change_set_review_state` a route, and decides the question RFC-020's Q3 deferred: whether a review decision is a record or an operation. Blocked on RFC-020 |
| 035 | [Change Detection Coverage and Disclosure](./proposed/035-change-detection-coverage-and-disclosure.md) | **Accepted 2026-08-18.** The `.git/hooks/` supervision hole and `max_changed_paths` discarding a computed list; the exit-only trigger and non-persistent baseline recorded and deferred |
| 036 | [Dormant Capability Closure](./proposed/036-dormant-capability-closure.md) | **Accepted 2026-08-18.** Wire / delete / document, per orphan from the reachability audit. Separates `recover` and `purge_all_records` as worse than dormant — recovery and deletion paths never exercised from the application |
...
| — | [Change detection wiring](./handoffs/change-detection-wiring.md) — no RFC; the last structural blocker on RFC-020. **Implemented 2026-08-18 (Slices A-D)**: one shared ignore-directory model; a real repository's measured baseline/detect cost (~5-6ms at 1,506 entries after exclusion); `add_detected_generated_change_set` has its first real production caller, wired at agent-run launch (baseline, captured before the process spawns) and exit (detection, via the previously-dormant `apply_agent_terminal_outcome`); a truncated scan's status is recorded distinctly from a genuinely clean one, never collapsing into the same "no `ChangeSet`" shape. Makes diff review **buildable, not reachable** — RFC-020's own surfaces remain unbuilt. Known, disclosed limitations: no mid-run change visibility for a long-lived interactive session (exit is the only trigger), and a captured baseline does not survive the application exiting mid-run |
...
| 031 | [Audit Producer Completion](./handoffs/031-audit-producer-completion/README.md) — the last M11 item; two families with live triggers and no producer |
