...
| RFC | Title | Status |
| --- | --- | --- |
| 041 | [Change Content Preview](./accepted/041-change-content-preview.md) | **Accepted 2026-08-25. M12, third of three for `0.14.0`.** `read_diff_content` and `gate_diff_content_read` were built, gated and reviewed in `0.7.0` and have **zero production callers**; `add_detected_generated_change_set` discards the `DetectedChanges` they need. Delivers **content preview, not a diff** — the before-bytes for a modified file were never captured, so a two-sided diff is blocked on Git-backed detection rather than cancelled. Three decisions settled on acceptance: session-scoped retention keyed by `ChangeSetId`; a stale baseline refuses and names why; `DiffContent` loses its unredacted `Debug`. [Handoff pack](./handoffs/041-change-content-preview/README.md) |
