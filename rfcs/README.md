...
| 037 | [Adopt the 5-Folder RFC Lifecycle](./proposed/037-five-folder-rfc-lifecycle.md) | **Proposed 2026-08-19** at the owner's direction. Adds `rfcs/accepted/`. RFC-000's own criterion is met — acceptance is a distinct owner act here — and its warning is inverted: `accepted/` would hold five files on landing while `proposed/` holds none. **Every file in `proposed/` today has Status Accepted**, so the folder, which RFC-000 makes the source of truth, currently misdescribes all five |
...
| — | [The leaked-child test flake](./handoffs/test-process-leak.md) — no RFC; cause known since 2026-08-16, three tests reported since, scheduled 2026-08-19 |
