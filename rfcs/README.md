...
| 039 | [Interaction Model and Visible Affordances](./accepted/039-interaction-model-and-visible-affordances.md) | **Accepted 2026-08-24. M12, after RFC-038**, ahead of RFC-020 and RFC-034. The product has no interaction design — 5 buttons application-wide, all inside two settings surfaces, **0** on the board a user arrives at, inert rows, no visible way back, and `close_project` reviewed in core with **no production caller**, so closing a project is a dormant capability rather than a missing button. Names the seven workflows and what each lacks, and adds the third reachability principle: naming a keystroke is not naming the path a user takes — name the control they see. Three decisions settled on acceptance: a project tab strip in the top bar (which persists in every mode, checked), confirm-on-close only when work is live (wiring `safe_close_decision`, unblocked at last), and the close confirmation identifying a project by canonical path rather than display name. [Handoff pack](./handoffs/039-interaction-model-and-visible-affordances/README.md) |
