...
| 031 | [Audit Producer Completion](./done/031-audit-producer-completion.md) | **Implemented and closed 2026-08-19.** `restricted_mode_blocked` and `project_added` have real producers, each proven from a real user path. The last M11 item. Does **not** make the audit store viewable — nothing renders it — and cannot say which of RFC-004's nine restricted features blocked a launch. `safe_close_decision` stays unwired, blocked on a dialog that does not exist |
