...
- **The reachability audit: results.** Scheduled 2026-08-17 (`rfcs/handoffs/reachability-audit.md`) after RFC-022 found two real shipped defects the moment `open_surface` got its first reader — dormant state is not merely untested, it is actively corrupting, because nothing audits its writers until something finally reads it.
  ...
  **The 30 true orphans, with what a user consequently cannot do:**
  ...
  | `runtime::terminal::launch::resize` | `runtime::terminal::launch` | **Implementation complete 2026-08-17, pending review (`rfcs/handoffs/terminal-resize.md`, review request 243).** At the time of this audit, no code anywhere called terminal resize -- `ROWS`/`COLS` were fixed constants shared by the spawned PTY's dimensions and the rendered grid, so every terminal was permanently 24×80 regardless of window size. `resize`/`resize_master` now have a real caller: `shell.rs` subscribes to `iced::window::resize_events()` for live drags and, per response 243's required fix, also queries the real window size once at boot (`iced::window::open_events()` plus `iced::window::size`) and re-applies the computed geometry from both production pane-launch call sites -- response 243's own finding was that the first pass sized a pane only on a live drag, so a pane launched before the user ever resized the window (the common case) stayed at the launch default forever. Every trigger computes rows/cols the same way and applies them through the single `TerminalPane::resize` that updates the PTY, the emulator grid, and the pane's own stored size together. **Not yet accepted** -- see the handoff and review request 243 for the review-gate evidence and the required-fix response for what changed since the first pass. |
  ...
  **Priority, per response 240's own re-ordering** (not simply "highest consequence first" — weighted by what a real user can reach today):
  ...
  1. **`resize`** — **implementation complete 2026-08-17, pending review**, see the row above.
     Was small and well-defined once the severity was pinned to "permanently 24×80," not a
     corruption risk; is now a real caller at both launch and live-resize, with review-gate
     evidence, pending the architect's acceptance of request 243's required fix.
