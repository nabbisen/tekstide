...
- **The reachability audit: results.** Scheduled 2026-08-17 (`rfcs/handoffs/reachability-audit.md`) after RFC-022 found two real shipped defects the moment `open_surface` got its first reader — dormant state is not merely untested, it is actively corrupting, because nothing audits its writers until something finally reads it.
  ...
  **The 30 true orphans, with what a user consequently cannot do:**
  ...
  | `runtime::terminal::launch::resize` | `runtime::terminal::launch` | **Discharged 2026-08-17 (`rfcs/handoffs/terminal-resize.md`, review request 243).** At the time of this audit, no code anywhere called terminal resize -- `ROWS`/`COLS` were fixed constants shared by the spawned PTY's dimensions and the rendered grid, so every terminal was permanently 24×80 regardless of window size. `resize`/`resize_master` now have a real caller: `shell.rs` subscribes to `iced::window::resize_events()`, computes each pane's target rows/cols from the window size and real font metrics (`TerminalPane::pane_dimensions_for_area`), and applies it through the single `TerminalPane::resize` that updates the PTY, the emulator grid, and the pane's own stored size together. See the handoff and review request 243 for the review-gate evidence (the three sizes proven to agree, an ablation showing the divergence a bypass produces, real-PTY rewrap at the new column count, and the resize-storm bound). |
  ...
  **Priority, per response 240's own re-ordering** (not simply "highest consequence first" — weighted by what a real user can reach today):
  ...
  1. **`resize`** — **discharged 2026-08-17**, see the row above. Was small and well-defined
     ...
     caller with review-gate evidence, pending the architect's review of request 243.
