...
**`NFR-PERF-004` (terminal input latency p95 ≤ 16 ms): structural cause removed, criterion not yet measured.** Not "met" — but **not undischargeable either, and this entry said otherwise for two days.**

**Corrected 2026-08-17.** This paragraph used to say that "met" requires an upper bound on the true end-to-end path "which includes compositor/GPU present cost," and therefore that the criterion "cannot be discharged under the current measurement discipline," leaving the owner an open choice between restating it and accepting it as permanently unverified. **There was never such a choice.** `NFR-PERF-004`'s own text excludes present time — *"Compositor and GPU present time are excluded, consistent with NFR-PERF-002 and NFR-PERF-003"* — made explicit on 2026-08-15 by RFC-017 Amendment 1, and `ARCHITECTURE.md` §Evidence conventions states the same rule project-wide, naming this exact failure: *"`NFR-PERF-004` was measured to pixels and declared unverifiable."* The requirement was corrected; this note was not, and it kept asserting the superseded reading. Found 2026-08-17 while answering an owner question that this stale note had itself prompted — the note created the dilemma it then asked the owner to resolve.

**So what actually remains is a measurement, not a decision.** The criterion is bounded at application state change, which is in-process and measurable without `iced::window::frames()`. Three live GUI attempts were confounded by shared-machine swap pressure and capped at three by review instruction; none were reported as clean numbers. Discharging this needs one run on a machine that is not swapping, under the bounded-output load condition the requirement names — not a new criterion, not new instrumentation, and nothing from the owner.

*(Kept from the earlier note because it is still true and still the reason `frames()` is not the answer: it **forces continuous redraw once subscribed** (RFC-015 §R1), so it replaces redraw-on-demand with redraw-always and changes the mechanism under measurement. If a future need for genuine presentation timing does arise — it does not arise for this criterion — Wayland's presentation-time protocol reports real present timestamps without forcing redraw, at the cost of going below `iced` and being platform-specific. Recorded so "unverifiable" is never written back in as a property of the world rather than of our tooling.)*

A headless, GUI-free benchmark proves the specific old cause is gone (sub-microsecond typical wake-to-`poll()` cost, ~500,000 real wakes/sec sustained, zero backlog). Three live GUI attempts across the amendment's own PR-A1-D were all confounded by the same shared-machine swap pressure PR-017-G's responses 155/156 first diagnosed, and were disclosed rather than reported as clean numbers, capped at three attempts per explicit review instruction.
...
- **A launch's retention limits are not retained past the launch, so "a running transcript is bounded by the configured retention" cannot be tested end to end.** Found at RFC-045 PR-045-B (2026-09-12) and reserved by the reviewer at response 377. `AgentRunLaunchPlan` carries `TranscriptRetentionLimits` into the launch and is then consumed; `AgentRun` does not hold them, and the `Transcript` attached afterwards records a fixed `retention_policy` **name** (`"local-bounded-agent-run"`), not the limits. RFC-045 D9's own test therefore reads the plan production builds — real, and one step short of a running transcript. Whoever widens this owns an RFC-011 data-model change (retain the limits on `AgentRun`, or make `Transcript::retention_policy` carry them), not a rider on another RFC's slice. It matters because the claim is one a user will eventually read in a changelog.
...
Text contrast is excellent — comfortably past AAA. **`border_default` fails WCAG 2.1 SC 1.4.11 (Non-text Contrast, AA), which requires 3:1 for UI component boundaries.** Focus indication is unaffected (`border_focused` is the accent at 6.38:1, and `NFR-UX-002` already pairs it with a non-colour channel), so this is the *unfocused* pane boundary being hard to see for a low-vision user, not a focus defect. **`theme/tests.rs` could never have caught it**: it asserts channels are "in range" and that heading is the largest font — type checks wearing the costume of quality checks. **Scheduled 2026-08-17** as `handoffs/theme-contrast-verification.md` — the gate first (landing red), then the fix; typography deliberately left out of it, per the owner. **Discharged 2026-08-18.** `theme/contrast.rs`'s three pure functions (validated against known anchors — 21:1 black-on-white, 1:1 identical colours, symmetry — before any claim about the real palette), a real-threshold test observed failing on `border_default` at the exact measured ratios above before the fix landed, then `border_default` raised to `0.45` (3.85:1 on `background`, 3.48:1 on `surface_elevated` — real headroom over the ~3:1 minimum). Does not establish the application is accessible; there is still no screen-reader support, and no light/high-contrast presets.
...
**On snora's accessibility APIs specifically** (asked 2026-08-17; the earlier "does not help our accessibility gap" was true only about screen readers and too broad as written). Three APIs, three different answers:

- **`snora_design::contrast` — genuinely useful, and it already proved itself**: the math in it is what found the `border_default` defect above. 81 LOC, three functions. The piece worth copying deliberately is **`composite_over`**: a translucent colour must be composited over its backdrop *before* its ratio is computed, which is exactly the case our `scrim` (alpha `0.55`) presents and the case a naive contrast test silently gets wrong.
- **`FocusTokens` — no, and snora documents why itself.** iced 0.14's `button::Status` has no `Focused` variant and `container` has no interaction status at all, so snora records its own focus ring as `BLOCKED (iced 0.14)`. **Tekstide is ahead of it here**, because it never delegated focus to iced: it owns `FocusZone`, and `zone_style` draws from its own `focused` boolean using **two channels — colour *and* border width (2.0 vs 1.0)** — which is what satisfies `NFR-UX-002`'s non-colour-only rule. Adopting `FocusTokens` would mean importing an API its own author marks inert. The same holds for **modal focus trapping**, which snora defers ("out of v0.20 scope") and RFC-018 already built here as security machinery with a positive control.
- **`Edge::Start` / `Edge::End` / `LayoutDirection` (their "ABDD" discipline) — the one genuinely novel API, and latent rather than live for us.** It replaces hardcoded Left/Right so layout mirrors correctly under RTL. Tekstide has **no layout-direction concept at all**, and ships `en` + `pl`, both LTR. Note this is a *different axis* from the bidi work already done here: `text_safety`'s escaping and i18n's First Strong Isolate/Pop Directional Isolate wrapping are about untrusted **text**, not layout mirroring. **Trigger to revisit: the first RTL locale.** Adding one against hardcoded edges would be a real gap; there is nothing to do before then.

**The most transferable thing snora has is not an API — it is the review gate**, and it is free. `accessibility-checklist.md` contains items this project would fail today: contrast asserted against real thresholds, translucent colours composited before measuring, rendering verified under a high-contrast preset, a minimum 24×24 pointer target, and a typography floor. **The `border_default` defect existed precisely because no such gate exists here.** An adapted checklist, run per surface, is the cheapest item in this whole evaluation.
...
**The decision stands, but on three legs rather than four.** Churn was one of four reasons and it is materially weakened — though not eliminated, and snora say so themselves rather than overclaiming: the covenant constrains *what may change*, not cargo's arithmetic, so `0.34` → `0.35` is still incompatible pre-1.0 and we would still bump every minor. The other three are untouched, and the first is **stronger** now than when written: we have since built and shipped our own contrast module (`55f53d8`), so adopting would mean deleting working, reviewed, ablation-tested code to take a dependency. `crate::theme::Theme` remains RFC-023's seam with security reasoning attached to individual roles, and `snora-widgets` remains out.

**Correction 2 — my own contrast table was under-enumerated, and it cost them a real defect.** I computed `border` against three surfaces but `text_muted` against **one**, then told them `text_muted` "already passes — so that one is free." Their reply computed the pair I skipped: **`text_muted` on `surface` in `light` is 4.46:1** and fails SC 1.4.3. Verified independently here. The role had gone untested for its whole life behind an invented exemption — *"exempt from the mandatory body-text contrast checks"* — that WCAG does not grant. My test-gap argument was right in a stronger form than I stated it.

**The same structural weakness is in our own test, and is worth closing.** `theme/tests.rs` enumerates 2 text pairs and 6 non-text pairs as a **hand-written literal list**. It is complete today — we have exactly two backdrops and every foreground role is checked against both — but nothing makes it grow. Add a third surface role or a new foreground role to `Theme` and the list silently stays the same size, which is precisely how `text_muted/surface` survived at snora. **Scheduled 2026-08-18** as `handoffs/derived-contrast-pairs.md`, after snora shipped the same fix (their RFC-063, 0.36.0) on the strength of our report: an exhaustive struct destructure so adding a field fails to compile until its usage is declared, with pairs derived from that declaration rather than hand-listed. Their coverage went 19 → 35 pairs and the change immediately caught two roles declared against nothing. **Our own gap for it to catch, now measured and a real failure**: the modal dialog's real backdrop is the scrim composited over what is behind it — including terminal content, which is arbitrary. I told snora we passed this "by luck"; that was reasoning, not measurement, and it was wrong. Over dark chrome the border identifies the card (6.92:1) and the fill does nothing (1.20:1); over bright content the fill identifies it (3.48:1) and the border does nothing (1.66:1); **at the crossing, terminal content around 0.78 grey, both measure 2.40:1** and the card has no boundary meeting SC 1.4.11. **Sampling the endpoints hides it entirely** — both extremes pass — so the pair must be asserted as a sweep over the backdrop range. RFC-018's spoofing evidence is unaffected: that rests on keystroke suppression and the scrim's existence, not the card border. Fix measured too: scrim alpha `0.75` gives 3.62:1 (`0.65` is not enough at 2.43:1), or a grey border at `0.75`+ gives 3.01:1. **Two method notes, both earned**: sweeping greyscale is sufficient — luminance is monotonic per channel and contrast depends on nothing else, so greys span every ratio a colour can produce (snora's point, verified here against a 400,000-sample 3D sweep) — and the sweep must be *solved*, not sampled: `max` of two monotone curves is unimodal, and a 2,000-step grid overstated our minimum by 0.0005, which at a different magnitude would pass a failing palette.

**Discharged 2026-08-18.** `theme/tests.rs::derived_contrast_pairs` exhaustively destructures `Theme` (no `..`) and declares every field's intended usage; a throwaway field added and removed during review confirmed `E0027` fires naming it, before the crate compiles. The **fixed-pair count did not go up (8 before, 8 after)** — the old hand-written list really was complete for the two backdrops that existed, not narrow; what changed is that a *future* field can no longer silently stay uncounted the way it could before. The modal-over-scrim backdrop cannot be expressed as a fixed pair at all (the failure lives strictly between the two sampled endpoints), so it is asserted separately: `contrast::minimize_unimodal`, a general ternary-search minimiser with its own anchor tests (a known parabola, a monotonic endpoint case, and a two-curve crossing), finds the true worst case by solving rather than sampling. Observed failing at exactly the measured number (`2.40:1` at `t=0.7844`) before the fix. Slice B raised the scrim alpha `0.55` → `0.75` (`3.62:1` at its own worst case), verified against the real rendered window (`niri msg action screenshot-window`) to confirm it is still visibly translucent, not only that the arithmetic says so — chrome behind the demo modal remained legible in the capture. `README.md`'s WCAG paragraph updated to describe both defects together.

**Also worth knowing, from their reply:**

- **`composite_over` is not exercised by anything snora ships** — they verified no built-in preset role is translucent (every colour is `Color::rgb`, not one `rgba`). Our alpha-`0.55` scrim is exactly the case it was written for, and our own implementation is now the tested one.
- **Their 24×24 pointer-target rule is asserted nowhere** — reviewed by eye. If we adopt their checklist, adopt it knowing which items have assertions behind them and which are aspirations; they were straightforward about the difference.
- **`focusable::find_focused()` requires iced's `advanced` feature**, which snora does not enable and therefore blocks their modal focus trapping. **We already enable `advanced`** (`iced = { version = "0.14", features = ["tokio", "advanced"] }`), so that route is open to us. We do not need it — `SubscriptionMode::for_modal` plus the `ModalAbsent` gate suppresses keystrokes outright, which is stronger than trapping focus — but the option exists and is worth remembering if the modal model ever changes.
- **They fixed both findings** in 0.34.0 (`border` to 3.12:1/3.17:1, `light` `text_muted` to 4.55:1, assertions written before values changed), moved the 3:1 non-text rule from a single role under *Focus visibility* into *Contrast* as a class rule, withdrew the invented exemption, and re-scoped the `BLOCKED` focus claim at all five sites. **If we copy their accessibility checklist, copy the current version, not 0.33.1's.**
...
- **The reachability audit: results.** Scheduled 2026-08-17 (`rfcs/handoffs/reachability-audit.md`) after RFC-022 found two real shipped defects the moment `open_surface` got its first reader — dormant state is not merely untested, it is actively corrupting, because nothing audits its writers until something finally reads it.
  ...
  **The 30 true orphans, with what a user consequently cannot do:**
  ...
  | `add_detected_generated_change_set` | `project::session` | Already known (item 5, re-verified): a generated change is never attached to a project session in production. **Scheduled 2026-08-17** (`handoffs/change-detection-wiring.md`) as the next theme, after `0.10.0` made agent-run launch reachable and left "see what it changed" as the last structural gap. **Scoping it surfaced a defect that would have made the wiring useless**: `scan_filesystem` has *no* ignore model — it walks `.git/`, `target/`, `node_modules/` — while `FileExplorerScanPolicy::linux_mvp` in the same crate already collapses exactly those three. With `max_entries: 4096`, a real project truncates almost immediately, and `detect_filesystem_changes` returns an **empty** `changed_paths` on any non-`Complete` status. Wired as-is it would have reported "no changes" for every real project, forever. **Measured 2026-08-17 on this repository** rather than left as reasoning (the original claim here was asserted without a number): **64,415 entries** walked in full, **1,506** after the three exclusions — ~15× over the cap, to comfortably under it. Slice A (`51b65bd`) closed the divergence with one shared `IGNORED_DIRECTORY_NAMES` constant. **Review response 251 found two follow-ups, both addressed before Slice C**: (1) the shared list was applied by *name* to any entry, not just directories, so a file or symlink named `target`/`.git`/`node_modules` was silently invisible to change detection the same way the real directory is — fixed to match the explorer's directory-only rule. (2) Excluding `.git/` is the right default (62,000 of the 64,415 entries measured above are inside it) but is a **known supervision blind spot**: an agent writing `.git/hooks/pre-commit` or editing `.git/config` will never show up in change review, and the explorer collapses `.git/` too, so there is no second route by which a user would notice. Whether Tekstide should watch a narrow, high-consequence subset of `.git/` (`.git/hooks/`, `.git/config`) is an open question, not scoped here. Slice C's own description must say "changed outside VCS metadata, build output, and package caches," not "what the agent changed" unqualified — this project has overstated reachability twice before in this same shape. **Slice C landed 2026-08-18**: `add_detected_generated_change_set` now has a real production caller. `attempt_agent_run_launch_with_profile` captures a real filesystem baseline at launch; `record_terminal_exit`, extended to look up and transition the owning `AgentRun`'s status (`ProjectSession::apply_agent_terminal_outcome`, `pub(crate)` with zero callers until this slice), runs detection against that baseline once the run's terminal exits and calls `add_detected_generated_change_set` for real. `max_entries` set explicitly to `16,384` per response 252's D4 decision, not inherited from `default()`'s `4,096`. Proven end to end from a real `Managed` launch, a real file write, a real socket-delivered approval decision, and a real process exit polled through the real `TerminalWoke` handler — not a dispatched command. **Makes a real `ChangeSet` buildable in production, still not diff review reachable** — RFC-020's own surface renders nothing. `max_changed_paths` and truncation-as-no-changes (D2) remain Slice D's. **Slice D landed 2026-08-18.** Review response 258 accepted Slice C and raised three items, addressed here: (1) **the launch/capture race, closed rather than shrunk** — the baseline is now captured *before* `launch_agent_run_with_runtime` spawns the process, not merely "as early as possible after," since a filesystem scan is not atomic with respect to a concurrent writer and a file modified mid-scan could have read as unchanged forever; `agent_run_id` is assigned onto the pre-captured baseline the moment it exists, which is what `detected_change_association`'s exact-match Strong-association check needs. (2) **D3's long-session question, unaddressed in the Slice C report, is a real, disclosed limitation, not a fixed one**: an interactive session left running for hours produces no change information until it exits — that is the only completion trigger this wiring has, deliberately, since a mid-run trigger is a real feature (polling cost, what "since when" means) that belongs with RFC-020, not built here. (3) **The captured baseline does not survive the application** — `agent_run_change_baselines` is an in-memory `HashMap` on GUI `State`; if Tekstide exits while a run is live, the baseline is gone and that run's eventual exit produces no `ChangeSet`, silently, from the outside indistinguishable from "the agent changed nothing." Bounded by `agent_run_limit` (not a leak), but a real gap. **D2 itself**: `state.agent_run_change_detection_status: HashMap<AgentRunId, ChangeDetectionStatus>` now records every detection attempt's real status, whether or not a `ChangeSet` results — the reason it exists is that `project.change_sets()` alone cannot tell "this run's changes are unknown, the scan was truncated" apart from "this run genuinely touched nothing," since both produce zero entries. Proven directly: one hand-crafted `Partial` baseline and one genuinely clean real run, in the same test, asserting both produce no `ChangeSet` **and** that their recorded statuses differ — ablated (temporarily removing the recording call) to confirm the test fails without it. No renderer reads this map yet — RFC-020 still does not exist — this is the data being honest before one does. **The open question above — whether to watch a narrow subset of `.git/` — is resolved: RFC-035 PR-035-A (`0.14.0`) decided yes, `.git/hooks/` and `.git/config` specifically, and built it.** `core.hooksPath` redirects are deliberately not followed to their target, recorded as its own, separate deferral rather than folded into this one. |
  ...
  | `transition_change_set_review_state` | `project::session` | **New.** No GUI lets a user mark a generated change set reviewed, accepted, or rejected — RFC-012's review workflow has no route. **Narrower since 2026-08-18**: `add_detected_generated_change_set` now has a real production caller (`change-detection-wiring` Slice C) and can populate a real, non-empty list — this gap is specifically the missing *review* route on top of it, not "nothing exists to review" any more. |
