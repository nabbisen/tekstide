...
- [ ] Non-contamination proven for this criterion by idle-CPU comparison — **attempted three times, results discarded**: this sandbox showed 54–57GiB of swap in use out of 59GiB during all three attempts (independently confirmed by the reviewer: 849MiB free RAM, identifiable unrelated consumers), an unrelated confound this test's own tiny footprint cannot explain, producing a reproducible but not-credible ~1.1–1.2 SECOND step in the raw samples (`qa-evidence.md`). Response 156 required the re-run to be made self-validating first (below) rather than re-attempted as-is; deferred to a quiet machine, does not block the verdict above, which is arithmetic.
- [ ] p50/p95/p99 and max reported; delivery loss reported; stopped on confirmed on-disk counts — **same confound, plus a second reading**: three live runs delivered all 1,100 samples with 0% loss and `dropped_bytes_total 0` — the reviewer read this as evidence the flood never reached rate *inside the application* (expected throughput ~17.2 MiB/s vs. a ~1.28 MiB/s drain capacity should have produced substantial drops), not as reassurance. **Response 156 required, both implemented (`b4e10ff`), not yet exercised**: observed in-app throughput (`TerminalPane::bytes_read_total`/`Measurement::elapsed`, printed at exit) as a precondition check the run's own output can fail; per-tick handler wall time (`Measurement::record_tick_handler`) as the discriminator between environment noise and a genuinely-overwhelmed update loop, meaningful regardless of machine load. A one-line environment snapshot (`/proc/meminfo`, every criterion) now makes every future run's log self-describing.
