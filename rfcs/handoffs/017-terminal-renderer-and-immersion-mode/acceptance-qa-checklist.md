...
- [x] **Dropped bytes under a genuine flood: `0`, with the mechanism now understood, not assumed.** The same headless benchmark measured observed in-app throughput at ~374KB/s against the flood script's own ~17.2MiB/s standalone rate (~46× lower) — the app's own draining rhythm (poll's ~10ms-dominated cost against a 50ms tick) throttles the flood via kernel pty backpressure, so no single bounded read ever accumulates enough to exceed the 64KiB cap. A real, disclosed characteristic of the current polling architecture, further motivating (not blocking) Option B as the long-term fix.
