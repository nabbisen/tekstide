...
**Dropped bytes stayed `0` even under a genuine, un-fork-bound flood — the reason is now known, not assumed, and it is a defect's consequence, not a "legitimate characteristic" (response 159's correction — an earlier draft of this section called it that, wrongly).** Arithmetic from the real numbers: 749,325 bytes over 40 polls ≈ 18.7KB per poll; each poll costs ~10.27ms, of which ~10ms is `read_available_bounded_for`'s `WouldBlock` sleep, leaving only ~0.27ms actually reading — so the reader sustains **~69 MB/s while it is reading**, for **~0.5% of each tick**, and sleeps through the rest. **The child process is not the limiter and the pty buffer is not the limiter — the 10ms sleep is**: it overshoots the loop's own 5ms budget and guarantees the loop exits after exactly one sleep, never taking a second read.

**User-visible consequence, stated plainly for PR-017-H's known limitations**: terminal output is capped around **~374KB/s** (roughly 7,500 lines/second) — fine for interactive use, but a verbose build, `cat` of a large log, or `find /` will hit this ceiling and the child process will block on its own `write()`. This is a real, disclosable limitation of the current architecture, and a **stronger, more concrete motivation for Option B than the latency arithmetic alone** — a user can see this one.

**The coupling that must be fixed together, not separately (response 159, the item most worth recording durably): fixing the 10ms sleep will unmask a real stream-truncation risk that the sleep currently, accidentally, masks.** The ~18.7KB-per-poll headroom below the 64KiB cap exists *only* because the sleep is starving the reader. Shorten or remove the sleep in isolation, and the loop would instead cycle read→brief-wait→read for the full 5ms budget; against the flood's real ~17MiB/s rate, a 5ms window offers ~85KB, plus buffered residue — comparable to the ~108,062 bytes per 5ms window response 155 independently measured from a completely different direction (the fork-free flood's own standalone throughput), which is why this projection is trusted rather than merely plausible. That exceeds the 64KiB cap, and `read_available_bounded_for` truncates the chunk at an arbitrary byte and keeps reading — feeding the VTE parser a byte stream with a hole in it. **This is not what P4 (stream-position independence) proves**: P4 proves classification is stable across arbitrary *chunking*, where every byte still arrives; a dropped chunk is a different property, never reachable at spike scale and still unproven here. **The sleep fix and the cap policy (block, grow, or drop-with-a-reported-count — currently silent truncation with the event discarded) are one change, not two.** This is `tekstide-core`'s `runtime::terminal` behavior, not specific to RFC-017's own scope — recorded here so the person who eventually fixes the sleep finds it before shipping a throughput fix that quietly turns into a stream-integrity regression.

Neither of the two paragraphs above is new work for this slice — both are disclosed, out-of-scope findings for whoever picks up Option B, consistent with (and a further motivation for) readiness-driven I/O as the real long-term fix.
...
**Terminal output throughput is capped around ~374KB/s, and the cause is a `tekstide-core` defect, not a design point** (PR-017-G, response 159). `read_available_bounded_for`'s `WouldBlock` branch sleeps a hardcoded 10ms against the caller's 5ms bound, so an idle-between-bursts poll blocks ~10ms and reads for well under 1% of each 50ms tick — measured, not estimated: ~69MB/s while actually reading, ~0.5% duty cycle. A verbose build, `cat` of a large log, or `find /` will hit this ceiling and the child process will block on its own `write()`. **Fixing the sleep and fixing the 64KiB output cap's silent-truncation policy must happen together, not separately**: the sleep currently, accidentally, keeps per-poll data volume (~18.7KB) well under the 64KiB cap; removing the sleep alone would let a real flood's ~17MiB/s rate exceed the cap almost every window, and `read_available_bounded_for` truncates at an arbitrary byte and keeps reading — feeding the VTE parser a stream with a hole in it, a property P4 (stream-position independence, about chunking, not dropping) does not cover and was never reachable at spike scale. Out of scope for RFC-017; `tekstide-core`'s `runtime::terminal` owns this.
