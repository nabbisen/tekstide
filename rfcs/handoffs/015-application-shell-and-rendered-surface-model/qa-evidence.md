...
Gates run 2026-07-31: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace --all-targets --all-features` (490 `tekstide-core` + 48 `tekstide` — up from 37, 11 net new — + 18 `tekstide-gui-spike`, 0 failures), `git diff --check` — all passed.
