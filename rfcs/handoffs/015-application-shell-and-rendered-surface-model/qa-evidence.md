...
**Screenshot evidence — deliberately not yet captured.** Per response 129's explicit instruction ("synthetic input needs flagging before you run it... raise it in the review request first"), showing the modal's Tab-cycling/dismissal working on screen needs synthetic keystroke delivery (`xdotool`), and RFC-014 found niri does not forward XTest to native Wayland clients — the established workaround is relaunching with `WAYLAND_DISPLAY` unset to force the X11/XWayland `winit` backend. This is raised here, before running it, rather than done and explained afterward, consistent with how the owner corrected the PR-015-B screenshot escalation.
...
- **Screenshot evidence for PR-015-C's modal dismissal/focus-trap is not yet captured.** Showing it working needs synthetic keystroke delivery (`xdotool`), which RFC-014 found requires relaunching with `WAYLAND_DISPLAY` unset (niri does not forward XTest to native Wayland clients). Flagged here, before running it, per response 129's explicit instruction.
