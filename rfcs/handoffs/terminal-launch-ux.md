...
- **A user can open a terminal, type in it, and see output.** Real, tested end to end (`launch_terminal_shell_input_switches_to_terminal_immersion_and_launches_a_real_session`): a `Ctrl+Alt+T` `ShellInput` dispatched through the real `update` switches to `TerminalImmersion`, launches exactly one real pane rooted in the **actual project directory** (not a scratch temp dir, unlike the diagnostic demo/measurement paths), and the session is already `Running`/`Primary` by the time it returns — not left at `Starting` the way every launch path did before this handoff.
  - **Screenshot**: [`terminal-launch-ux-evidence.png`](terminal-launch-ux-evidence.png), taken 2026-08-08 against the release binary via a real, focused window and individually-dispatched `xdotool` keystrokes (`Ctrl+Alt+T`, then `e c h o SPACE h i Return` sent one key event at a time, never a pre-seeded string). **What it proves**: `Ctrl+Alt+T` opens a real session (`Terminal 1 (Primary) — Running`), and the pane echoes `echo hi` → `hi` from genuinely separate keystrokes through the real `write_input`/PTY/`poll()`/filter path. **What it does not prove**: trusted-UI separation or spoofing resistance (RFC-018's, unchanged).
  - **Provenance defect, disclosed rather than hidden**: the launch that produced this screenshot ran against the real `XDG_STATE_HOME` (`~/.local/state/tekstide/`) through an operator error — a scratch `XDG_STATE_HOME` was intended and dropped from the launch command on a retry — not the scratch directory the review-gate plan called for. This is why the status bar reads `Project Workspace | 8 projects` instead of a fresh scratch board; that count is the only real-state detail visible in the image, and it carries no project names or paths. The defect is orthogonal to what the image demonstrates — which directory holds `recent-projects.json` has no bearing on whether a real keystroke reached a real PTY — and is accepted on that basis per review response 164, rather than retaken. Full incident detail, blast radius, and remediation status: review request/response 164.
- **The session limit is enforced in core and demonstrated, with what the user sees on refusal.** `ProjectSession::add_terminal_session` refuses at `terminal_session_limit` (`Some(3)`, was `None`) with `ProjectTerminalError::SessionLimitExceeded { limit }`; `terminal_session_limit_is_enforced_end_to_end_with_a_visible_notice` runs 3 real launches to the limit, confirms the 4th typed refusal, and confirms the catalog-driven notice text (`terminal-launch-refused`, `en.ftl`) names the real number. Ablated (`ablation_a_fourth_real_process_would_spawn_without_the_limit_check`): a 4th real shell spawns just fine at the OS level, confirming the application-level check is the only thing that stops it.
  ...
  - **Known limitation, recorded 2026-08-08 per review response 165**: three is not generous for a developer workbench -- a user with a build running, a log tailing, and a shell open is already at the limit. This is not a mistuned number; it is the poll-cost defect above becoming a **user-visible** limitation for the first time, rather than an internal latency/throughput note nobody outside the codebase sees. The limit is expected to rise once readiness-driven terminal I/O (`rfcs/future-work.md` §Readiness-driven terminal I/O) removes the per-poll sleep it is a function of -- raising it before that fix would reopen the tick-saturation risk this default exists to prevent.
...
**Not yet done**: the non-contamination/GUI-only items that were never this handoff's to begin with. Everything else in the review gate is real, tested, ablated where the gate asked for ablation, and — as of the screenshot above — captured.
