...
itself. **Still open, and explicitly PR-043-C's own scope, not this fix's**: the close-confirmation
dialog's wording, and `terminal_process_groups_confirmed_empty`'s own rename/rewiring in
`shell.rs` -- the mechanism kills the job; the user-facing surfaces have not yet been told the
mechanism changed.
