...
- [ ] Token delivered through the environment allowlist only. `bind()` returns the raw token string for a launcher to deliver; the actual environment-allowlist injection into the spawned adapter process is launch-path wiring owned by PR-021-E, not implemented in this slice.
