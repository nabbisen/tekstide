...
| --- | --- | --- | --- | --- | --- | --- |
| C2 typing | 0ms | 0ms | 0ms | 0ms | p95 ≤16ms, p99 ≤33ms | **Not verified**, see caveat above |
| C3 terminal-under-flood | 0ms | 0ms | 0ms | 0ms | p95 ≤16ms | **Not verified**, see caveat above |
| C4 mode switch | 0ms | 0ms | 0ms | 0ms | p95 ≤32ms | **Not verified**, see caveat above |
...
**The instrumentation limitation above is specific to C2, C3, and C4. C5, C6, C7, and C10 below are clean, uncontaminated results and should be read as such** — none of them depend on the degenerate "time to next `Frame`" signal, for the reasons stated at each: C5 only ever times a single first frame, C6 never subscribes to `frames()` at all, and C7/C10 use entirely different mechanisms (headless text layout; screenshots). The caveat above must not be read as tainting these four.
...
- **Terminal surface, no bidi reordering — correct behaviour, not a gap.** The Arabic line appears in raw left-to-right *cell* order (the order bytes were written to the grid), not visually reordered right-to-left the way the editor surface shows it. This is expected, not a defect: real terminal emulators (this one included, via `alacritty_terminal`'s grid model) operate on a monospace cell grid rather than shaped text runs, and generally do not implement the Unicode bidi algorithm at all. Stated positively so this is not later filed as a bug: a terminal-grid renderer choosing not to bidi-reorder is normal, industry-standard behaviour, and CJK/Arabic glyphs still shape and render correctly at the individual-glyph level (Arabic letters still join/connect properly within each printed line) — only the cell *ordering* is unreordered.
- **Terminal surface, no wide-cell CJK — a real rendering-correctness gap.** CJK characters occupy exactly one grid cell each here, not the two cells a real terminal emulator gives "wide" (fullwidth) characters. `alacritty_terminal`'s grid supports wide-character cells; this spike's minimal rendering path (`terminal_pane.rs::styled_rows`) was not built to consume that distinction, so CJK text is visually mangled (column-misaligned) in the terminal surface as rendered here. Unlike the bidi point above, this one is a gap that should be fixed, not a correct design choice.
- **Design fact for RFC-016 (i18n) and RFC-017 (terminal renderer): the two surfaces now have measurably different i18n fidelity.** The editor surface reorders bidi correctly and has no wide-cell issue; the terminal surface does neither, by the nature of the grid-rendering approach. This asymmetry is real and exercised, not speculative, and is recorded here as an input to both RFCs rather than left to be rediscovered.
...
- **Methodology limitation for any future reuse of this harness (per response 108 Q2), not a defect of this run's result:** computing percentiles over confirmed-received samples only (rather than a fixed sent count) correctly avoids padding for missing samples, but it does not rule out **survivorship bias** if delivery loss is non-random — specifically, if drops correlate with the app being busy, which is exactly when latency would be highest. At 12.78% loss on C2, that would be a material fraction if this harness's numbers were ever load-bearing. It does not affect the present result (the C2/C3/C4 measurement is already degenerate for the unrelated continuous-redraw reason above), but M8 reuse of this harness on a non-degenerate instrumentation path must account for this before treating its percentiles as unbiased.
