...
| `gpui` | Passes — Apache-2.0. | **Concerning.** Zed Industries paused community-facing GPUI development in 2026 ("hard for Zed Industries to justify work on GPUI that is purely for the community"); community fork `gpui-ce` has single-digit merged PRs and is ~381 commits behind mainline (per Hacker News discussion of the announcement). | **Strongest of the shortlist** — Zed itself ships a production terminal panel built on GPUI. | Screened out on C14 despite the best precedent: bus-factor risk from an explicitly deprioritized steward is too high for a small team's multi-year dependency |
