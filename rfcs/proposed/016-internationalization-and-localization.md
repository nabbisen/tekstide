...
- **Over-escaping harms legitimate RTL users.** Genuine Arabic or Hebrew UI text must render naturally; only *untrusted* spans are escaped. Mitigation: the trusted/untrusted distinction is per-span, not global, and the test corpus must include legitimate RTL content rendering unescaped in chrome. **Verified 2026-07-30** (PR-016-C, review response 121): Arabic, Hebrew, Devanagari with combining marks, and CJK all pass through `escape_untrusted_chars` unchanged.
- **Over-escaping does affect emoji sequences and variation selectors, and this is accepted rather than mitigated.** Measured 2026-07-30 (PR-016-C, review response 121): a ZWJ emoji sequence renders as `👨<U+200D>👩<U+200D>👧`, an emoji-presentation sequence as `❤<U+FE0F>`, and Mongolian free variation selectors as `ᠠ<U+180B>` — because `U+200D`, `U+FE00..U+FE0F`, and `U+180B..U+180D` are all in the escape set (`Cf` and `Default_Ignorable_Code_Point` respectively). This is the correct direction for a *security* surface, where a joiner or variation selector can alter what a user believes they are approving. But it applies to every untrusted span, including informational ones — a project directory named with an emoji shows visible markers in a Project Board row. **Do not "fix" this by narrowing the escape set.** If it needs addressing, the correct instrument is a per-surface policy distinguishing security-critical surfaces (approval dialog, trust prompts) from informational ones, which this RFC deliberately does not define. Recorded so a future UI review does not file it as a defect.
