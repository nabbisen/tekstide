...
Status: Implemented — closed 2026-08-11 by PR-019-E, all five slices accepted (responses 180, 181, 182, 183, and this closeout). Content mode renders for real: a trusted-chrome explorer tree (every name/path/status escaped, bidi-override tested and ablated) and a cursor-aware editor over `TextDocument` (text area raw and bidi-reordered, chrome escaped, ablated in the opposite direction) with real save and conflict handling. **RFC-006 Amendment 1** (recorded in that RFC's own text) added the one accessor core was missing — a cursor-only forwarding method — closing PR-019-D's own escalation rather than shipping the append-only editor it would otherwise have been stuck with. **The conflict dialog distinguishes a genuine conflict from a clean document that merely changed on disk**, found live during closeout (a real save was claiming local changes would be discarded when there were none) and fixed, not merely disclosed. **Not claimed:** syntax highlighting, language servers, multi-cursor, search-and-replace, or undo history beyond RFC-006's own model — all non-goals, unchanged by implementation. **Known limitations, not defects:** a mid-buffer edit is unrecoverable within the session past what Backspace can still reach (no undo); files above 4 MiB remain unopenable for editing (the existing policy bound, not a new one); the explorer shows a symlink's status but not its target. See `../handoffs/019-editor-and-explorer-surfaces/qa-evidence.md` §PR-019-E.
