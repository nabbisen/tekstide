...
- [RFC-020](../done/020-diff-review-and-agentrun-report.md) — the diff review surface has nothing to render until this exists.
...
2. **Does a stale baseline invalidate the whole review or one path?** Whole-review is safer and cruder; per-path is more useful and needs a per-path staleness check. Decide against the real cost of the check.
   **Answered (PR-024-D, 2026-08-11): per-path.** This RFC's own content-access model (PR-024-B/C) is already per-path — one `read_diff_content` call per selected file, per Decision 1 clause 2's on-demand rule — so a per-path staleness check matches the shape the rest of this RFC already committed to; a whole-review mechanism would need either a fresh re-scan of the entire `DetectedChanges` set (RFC-012's own job, out of this RFC's scope) or a separate review-level flag answering a question no user actually asked (they opened one file, not "the review"). The real cost, per the question's own instruction to weigh it: one extra bounded metadata call (`fs::metadata`, no content read) per staleness check — the same cost class already accepted for the original content read, not a new expensive operation. `diff_content_is_stale` reuses `content::FileSnapshot`, comparing `len`/`modified_at` only (not `content_hash`, deliberately never computed here — hashing would mean a second full read of every file, or a read of binary content `NonTextContent` exists specifically to avoid), matching the granularity RFC-012's own `changed_paths_between` already uses for the same before/after question.
